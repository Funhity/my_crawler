...
class WepayNotifyCallBackApp extends WxPayNotify {
    ...
    public function NotifyProcess($data, &$msg)
    {
        ...
        $pay_type = Paycache::PAY_TYPE_VOTE;
        $CI =& get_instance();
        ...
        $vote_id = $CI->weapp_vote_order->getVoteId($order_id);
        $alloction_result = $CI->weapp_vote->alloction($vote_id);
        if($alloction_result){
            pushlog('分配成功', $vote_id);
        }else{
            pushlog('分配失敗', $vote_id);
        }
