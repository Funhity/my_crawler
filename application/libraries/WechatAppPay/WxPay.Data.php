...
* 2015-06-29 修复签名问题
**/
require_once "WxPay.Config.App.php";
require_once "WxPay.BMYY.Config.App.php";
require_once "WxPay.AK.Config.App.php";
require_once "WxPay.KING.Config.App.php";
require_once "WxPay.PIC.Config.App.php";
require_once "WxPay.GOD.Config.App.php";
require_once "WxPay.HAPPY.Config.App.php";
...
class WxPayDataBase
{
	...
	**/
	...
	public function MakeSign()
	{
		//签名步骤一：按字典序排序参数
		ksort($this->values);
		pushlog('sign  key values: ', $this->values);
		$string = $this->ToUrlParams();
		//签名步骤二：在string后加入KEY
		$config = WxPayApi::$wxPayConfig;
		$string = $string . "&key=".$config::KEY;
		pushlog('sing string values: ', $string);
		//签名步骤三：MD5加密
		$string = md5($string);
		//签名步骤四：所有字符转为大写
...
class WxPayResults extends WxPayDataBase
{
	...
	public static function InitWithdrawResponse($xml)
	{	
		$obj = new self();
		$obj->FromXml($xml);
	...
	public static function &InitDownloadbillResponse($response, $fields_pos=null)
	{
...
交易时间,小程序ID,商户号,子商户号,设备号,微信订单号,商户订单号,用户标识,交易类型,交易状态,付款银行,货币种类,总金额,代金券或立减优惠金额,微信退款单号,商户退款单号,退款金额,代金券或立减优惠退款金额,退款类型,退款状态,商品名称,商户数据包,手续费,费率
...
*/
		...
		$tmpFile = APPPATH . 'logs/' . uniqid('downbill_');
		...
		$last_first_line_data = [];
		$last_second_line_data = [];
		
		$i=0;
		while ( ($ld = fgetcsv ( $handle, 8094, "," )) !== FALSE ) {
			$i++;
			...
			if ($i == 1) {
				...
				if(!$fields_pos){
					...
					$last_second_line_data = $last_first_line_data;
					$raw_ld = $last_first_line_data = $ld;
					$ld = [];
					foreach($fields_pos as $p){
						$v = $raw_ld[$p];
		...
		if(!$fields_pos){
			$stat_values = array_pop($data['list']);
			$stat_fields = array_pop($data['list']);
		}else{
			$stat_values = $last_first_line_data;
			$stat_fields = $last_second_line_data;
		}
		
		foreach($stat_fields as $p=>$f){
			$v = $stat_values[$p];
			if (substr ( $v, 0, 1 ) == '`') {
				$v = substr ( $v, 1 );
			}
 ...
 * @author 炜杰
 */
class WxWithdrawDeposit extends WxPayDataBase{
	...
	public function GetCheck_name($value){
...
class WxPayJsApiPay extends WxPayDataBase
{
	...
	**/
	 ...
	 * App支付设置PrepareId:
	 **/
	public function SetPrepareId($value)
	{
	...
	{
		$config = WxPayApi::$wxPayConfig;
		$this->values['partnerid'] = $config::MCHID;
	}
	// ###########################################
