...
class WxPayApi
{
	...
	public static function unifiedOrder($inputObj, $timeOut = 6)
	{
		...
		if(!$inputObj->IsNotify_urlSet()){
			throw new WxPayException("缺少回调地址");
		}
		
		if(!$inputObj->IsAppidSet()){
			throw new WxPayException("缺少对应APPID");
		}
		$config = self::$wxPayConfig;
	 ...
	 */
	public static function withdrawDeposit($inputObj, $timeOut = 6){
		...
		pushlog('withdraw response', $response);
		$result = WxPayResults::InitWithdrawResponse($response);
		self::reportCostTime($url, $startTimeStamp, $result);//上报请求花费时间
	 ...
	 * @return 成功时返回，其他抛异常
	 */
	public static function downloadBill($inputObj, $timeOut = 6, $fields_pos=null)
	{
		...
		return WxPayResults::InitDownloadbillResponse($response, $fields_pos);
	}
	...
	{
	    /*
		empty($GLOBALS['HTTP_RAW_POST_DATA']) && die();
		//获取通知的数据
	...
	{		
		...
		if($data){
			...
			$error = curl_error($ch);
			curl_close($ch);
			throw new WxPayException("curl出错:$errno-$error");
		}
	}
