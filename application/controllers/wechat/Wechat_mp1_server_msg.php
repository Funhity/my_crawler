...
/**
 * @api {post} wechat/Wechat_mp1_server_msg 01. 公众号消息接口
 * @apiName wechat_msg_Wechat_mp1_server_msg
 * @apiVersion 1.0.0
 * @apiGroup WechatMp
 *
 * @apiDescription 微信公从号(Wonder知识共享)消息接口对接<br />
 * <br />
 ...
 * 微信公众号消息对接接口
 */
class Wechat_mp1_server_msg extends MY_Controller {

    private $request;
    private $wx_interface_debug;
    private $weCfg = null;                      // 公众号配置文件
    private $admins = array(
        'oXWLN1eQfMwhTwknGngbJcSplH00',         // 官海升
...
//        pushlog('receive raw data: ', $GLOBALS['HTTP_RAW_POST_DATA']);
        ...
        $interface_token = 'ljDsdfMQljsdlfjdDjiwjGe2834W8n2w';		// 微信消息接口配置密码
        if ($this->isValid() && $this->validateSignature($interface_token)) {
            exit($_GET['echostr']);
        }
        $this->wx_interface_debug = FALSE;
        //set_error_handler(array(&$this, 'errorHandler'));		// 打开这行无法同时发送客服消息与回复消息，待解决
        // 设置错误处理函数，将错误通过文本消息回复显示
        $xml = (array) simplexml_load_string($GLOBALS['HTTP_RAW_POST_DATA'], 'SimpleXMLElement', LIBXML_NOCDATA);
        $this->request = array_change_key_case($xml, CASE_LOWER);

        $this->weCfg = Consts::WECHAT_MP1;
        ...
        if(in_array($msgType, ['text', 'voice', 'video', 'shortvideo', 'image', 'link']) || ($msgType == 'event' && in_array($event, ['subscribe', 'CLICK', 'VIEW', 'SCAN'])) ) {
            $activeKey = ENVIRONMENT.':WECHAT:MP1:ACTIVE_OPENIDS:'.$this->getRequest('fromusername');
            $redis = RedisInstance::getInstance();
            $redis->setex($activeKey, 172800-180, date('Y-m-d H:i:s'));      // 两天内为激活用户, 2天减3分钟
        }


        // 获得这个openid对应的用户信息: 【Wonder知识共享】公众号是后面迁移过来的，这个表中可能没有这个微信公众号的用户信息;
...
//              pushlog('模板消息内容: ', $template);
    ...
    private function onSubscribe() {
        ...
        $this->wechat_user->mpSubscribeCheck($this->getRequest('fromusername'), 1, $eventkey);
        ...
        $this->defaultRespMsg();        // 没有带场景二维码，返回默认消息;
    }
    ...
    private function onUnsubscribe() {
        ...
        $this->loadModel('user/wechat_user');
        $this->wechat_user->mpSubscribeCheck($fromuser, 0);     // 取消关注，更新用户表;
    }
    ...
    private function onClickMenuClick() {
        ...
        switch ($eventKey) {
            case 'key_2_4':
                exit($this->responseText('收到菜单点击'));			// 老炮聊互联网
                break;
            default:
    ...
    private function onQrScan($eventKey=null) {
        ...
        // 检查用户是否进行系统注册
        $this->loadModel('user/wechat_user');
        $this->wechat_user->userSystemRegisterCheck($fromuser, Consts::WECHAT_MP1, $eventKey);

        // 如果传递进来的场景值是一个大于6位的数字，并且是以10开始的，那么这是一个live分享海报的扫描事件，交到海报处理程序去处理:
        if(is_numeric($eventKey) && strlen($eventKey) > 6 && substr($eventKey, 0, 2) == 10) {
            $this->load->model('live/live_invite_scan');
            $this->live_invite_scan->processUserScan($fromuser, $eventKey);
            exit('');       // 返回一个空字符串给微信:
        }


        // 用户从Live海报进来报名: qr格式: qr_live_sign_[live_id]
        $keyArr = explode('_', $eventKey);
        pushlog('arrayKey: ', $keyArr);
        if($keyArr[1] == 'live' && $keyArr[2] == 'sign') {
            $this->liveBookQrscanProcess($eventKey);
        } else if($keyArr[1] == 'live' && $keyArr[2] == 'card') {           // 海报图生成操作:   qr_card_[live_id]
            $this->loadModel('live/live_invite_scan');
            $live_id = $keyArr[3];
            $this->live_invite_scan->generateCardForUser($live_id, $fromuser);
            exit($this->responseText('正在为您生成海报...'));
        ...
        } else if($keyArr[1] == 'wegroup' && $keyArr[2] == 'tmpact170406') {
            $this->weixinkefu->sendText($this->getRequest('fromusername'), '欢迎关注wonder知识共享，我们极力提供优质海外知识共享内容。
 「获取英语四六级资料，请扫码入群」');
            exit($this->responseImage("nR_dxCyq0EwW4o0hu5h0E7Zp6-WkRHMj8xLa5M_7Szw"));
        }
        // 如果执行上面的请求还没有退出，那么执行默认消息并退出;
        $this->defaultRespMsg();        // 返回默认消息:
    }
...
ERR;
    ...
    // 默认返回消息:
    private function defaultRespMsg() {
        $text = "We are very happy to have you. We will invite foreigners/experts to share their knowledge/experience with live voice in English, It's a good way to improve our English skills :) Here is the link to listen: Public English Speaking Skills - By Jonathan from the US

http://web.dolphinlive.net/h5/wd_live/api_detail?live_id=L201703091126236423";
        exit($this->responseText($text));
    }
